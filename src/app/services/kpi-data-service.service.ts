...
export class KpiDataService {
  ...
  private keyresultUrl = 'http://127.0.0.1:8000/kpis/';  // URL to web api
  ...
  getKpis(
    ...
    return this.http.get<any>(url, {
      ...
      catchError(this.handleError<Kpi[]>('failed to getkeyresult', [])),
      map(response => response["results"]),
    );
  }

  /** GET keyresult by id. Return `undefined` when id not found */
  getKpiNo404<Data>(id: number): Observable<Kpi> {
    ...
    return this.http.get<Kpi[]>(url)
      .pipe(
        ...
        catchError(this.handleError<Kpi>(`getkeyresult id=${id}`))
      );
  }
  ...
  searchKpi(term: string): Observable<Kpi[]> {
    if (!term.trim()) {
      // if not search term, return empty keyresult array.
      return of([]);
    }
    return this.getKpis(term);
  }

  //////// Save methods //////////

  /** POST: add a new keyresult to the server */
  addKpi(keyresult: Kpi): Observable<Kpi> {


    //TODO - Add error handling for name and description

    return this.http.post<Kpi>(this.keyresultUrl, keyresult).pipe(
      tap((newkeyresult: Kpi) => this.log(`added keyresult w/ id=${newkeyresult.id}`)),
      catchError(this.handleError<Kpi>('addkeyresult'))
    );
  }

  /** DELETE: delete the keyresult from the server */
  deleteKpi(id: number): Observable<Kpi> {
    const url = `${this.keyresultUrl}/${id}/`;

    return this.http.delete<Kpi>(url, this.httpOptions).pipe(
      tap(_ => this.log(`deleted keyresult id=${id}`)),
      catchError(this.handleError<Kpi>('deletekeyresult'))
    );
  }

  /** PUT: update the keyresult on the server */
  updateKpi(keyresult: Kpi): Observable<any> {
  ...
  private log(message: string) {
    console.log(`KpiDataService: ${message}`);
    this.messageService.log('KpiDataService',`${message}`);
  }
}
