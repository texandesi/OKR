...
import { organizationsHooks, usersHooks, groupsHooks } from "@/hooks/useApi";
...
export function OrganizationsList() {
  ...
  const [formData, setFormData] = useState<OrganizationCreate>({ name: "", description: "" });
  ...
  const { data, isLoading, refetch } = organizationsHooks.useList({
    page,
    pageSize,
  ...
  const createMutation = organizationsHooks.useCreate();
  const updateMutation = organizationsHooks.useUpdate();
  const deleteMutation = organizationsHooks.useDelete();
    ...
    <div className="space-y-4">
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>
              {editingItem ? "Edit Organization" : "Add Organization"}
            </DialogTitle>
          </DialogHeader>
